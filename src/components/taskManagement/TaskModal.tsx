...
export default function TaskModal({ 
    ...
    if (isOpen) {
      ...
      if (task) {
        setFormData({
          ...
          statusId: typeof task.status === 'string' ? task.status : task.status.id,
          priorityId: typeof task.priority === 'string' ? task.priority : task.priority.id,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 shadow-xl w-full overflow-y-auto h-[100dvh] sm:h-auto sm:max-h-[90vh] sm:max-w-2xl sm:rounded-lg">
